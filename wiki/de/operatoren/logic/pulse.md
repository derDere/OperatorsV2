...
einen **Impuls** — ein An, das nur einen einzigen Rechenschritt lang
dauert — sobald der Eingang umschaltet: auf `U` beim Einschalten, auf `D`
beim Ausschalten. Den Moment des Einschaltens nennt man die **steigende
Flanke**, den des Ausschaltens die **fallende Flanke**; wie lang ein
Rechenschritt ist (ein **Tick**), erklären
[Werte und Signale](../../grundlagen/werte-und-signale.md) und
...
| --- | --- | --- | --- |
| `I` | Eingang | Bit | Der beobachtete Wert |
...
- **„Nur einmal pro Einschalten"**: Ein dauerhaft anliegender Wert würde
  manche Aktionen ununterbrochen auslösen. Pulse verwandelt ihn in einen
  einzelnen Anstoß — perfekt vor Auslöser-Eingängen von
  [Stack](../memory/stack.md), [Memory](../memory/memory-1byte.md) & Co.
  (Viele dieser Bausteine reagieren allerdings schon selbst nur auf die
  steigende Flanke — der Tooltip des Eingangs verrät es.)
