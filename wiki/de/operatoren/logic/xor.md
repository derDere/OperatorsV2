...
Xor (sprich: „Exklusiv-Oder") ist der **Unterschieds-Melder**: Sein
Ausgang ist genau dann an, wenn die beiden Eingänge **verschieden** sind
— einer an, der andere aus. Sind beide gleich (beide an oder beide aus),
bleibt er aus. Wie eine Treppenbeleuchtung mit zwei Schaltern, einem oben
und einem unten: Nur wenn beide unterschiedlich stehen, brennt das Licht.

„Exklusiv" heißt hier „nur eins von beiden, nicht beide zusammen" — genau
das prüft dieser Baustein.
...
- **Nibble / Byte**: vier bzw. acht Eingänge. Variant „Combined" macht
  ...
  Eingängen an ist. „Channeled": unabhängige Zweier-Bausteine.
...
Wert von `I1` durch, solange `I2` aus ist — und **dreht ihn um** (man
sagt auch: **invertiert** ihn), solange `I2` an ist. Ein Not mit
Ein-/Ausschalter. Im Modus Bitwise legst du auf `B` eine Zahl, deren
eingeschaltete Bits angeben, welche Bits von `A` gekippt werden — so eine
Auswahl-Zahl nennt man **Maske**.

**Rechnen: der Halbaddierer.** Xor ist die Einer-Stelle beim Addieren
zweier Bits, And der **Übertrag** — die Merkzahl, die man beim
schriftlichen Addieren in die nächste Spalte schreibt (mehr dazu in
[Negative Zahlen und Überlauf](../../grundlagen/negative-zahlen-und-ueberlauf.md)).
Zusammen ergeben
Xor und And einen Halbaddierer, die kleinste Rechenmaschine der Welt. Die
beiden Kästchen links sind die Bits A und B (jedes speist über eine
kleine Weiche gleichzeitig das Xor und das And). Rechts zeigt
„Xor · Output" die Einer-Stelle und „And · Output" den Übertrag —
schalte beide Bits an, um den Übertrag zu sehen:
...
nur Einer-Stelle an = 1, nur Übertrag an = 2. Schreibt man Übertrag und
Einer-Stelle nebeneinander, steht dort für die 2 das Muster `10` — zwei
Bits, gelesen wie in
[Bits und Bytes](../../grundlagen/bits-und-bytes.md) beschrieben.
