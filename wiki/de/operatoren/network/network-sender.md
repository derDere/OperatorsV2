...
einen benannten **Kanal** — genau wie bei einem Walkie-Talkie, bei dem
alle mit demselben eingestellten Kanal sich gegenseitig hören. Der Kanal
lebt auf dem **Server**: dem Computer im Hintergrund, der alle Sender und
Empfänger miteinander verbindet. Alles, was auf denselben Kanal hört —
ein [Network Receiver](network-receiver.md) in derselben Schaltung, in
einem anderen Browser-Tab oder auf einem anderen Rechner — bekommt den
Wert. So verlassen Signale erstmals die eigene Zeichenfläche.
...
Ein Kanal ist flüchtig wie Funk: Senden mehrere gleichzeitig, werden die
Bytes bitweise mit **ODER** zusammengelegt — jedes einzelne Bit zählt als
„an", sobald es bei mindestens einem Sender an ist (mehr dazu unter
[Werte und Signale](../../grundlagen/werte-und-signale.md)). Ohne neue
Sendungen verklingt der Wert nach kurzer Zeit von selbst auf 0.
...
| `T` | Eingang | Bit | [Steigende Flanke](../../grundlagen/flanken-und-takt.md) (der Wechsel von aus zu an): sendet den aktuellen Wert |
