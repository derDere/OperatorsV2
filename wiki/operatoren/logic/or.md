...
Das Or-Gatter ist die „Oder"-Bedingung: Sein Ausgang ist an, sobald
**mindestens einer** der Eingänge an ist. Wie ein Flur mit zwei
Lichtschaltern — egal welcher gedrückt wird, das Licht geht an.
...
- **Nibble / Byte**: vier bzw. acht Eingänge. Variant „Combined": an,
  sobald irgendeiner an ist; „Channeled": unabhängige Zweier-Gatter.
- **Bitwise**: zwei Bytes `A` und `B`, das Oder wird je Bit-Stelle
  gerechnet — die Bits beider Zahlen werden also **vereinigt**.

## Einsatzideen

- **Sammel-Melder**: Mehrere Auslöser (Fenster 1 offen, Fenster 2 offen …)
  führen auf einen gemeinsamen Alarm.
- **NOR bauen**: Der Ausgang `!O` ist ein fertiges NOR — an nur dann, wenn
  gar nichts an ist. Praktisch als „Ruhe-Anzeige":
...
- **Bits setzen** (Modus Bitwise): Mit einer festen Zahl auf `B` schaltest
  ...
  das Gegenstück zur Bit-Maske des [And](and.md).
- Übrigens: Führen mehrere Leitungen auf **denselben Eingang**, verhalten
  ...
  explizites Or-Gatter macht diese Absicht aber sichtbar und hält die
  Schaltung lesbar.
