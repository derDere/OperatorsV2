...
Das And-Gatter ist die „Und"-Bedingung: Sein Ausgang ist genau dann an,
wenn **beide** Eingänge an sind. Wie eine Tür mit zwei Schlössern — sie
öffnet nur, wenn beide Schlüssel gedreht werden.
...
- **Sicherheits-Bedingung**: Etwas darf nur passieren, wenn mehrere
  ...
  Bremse gedrückt.
- **Tor für Signale**: Lege ein Taktsignal auf `I1` und eine „Freigabe" auf
  `I2` — der Takt kommt nur durch, solange die Freigabe an ist. So lassen
  sich ganze Schaltungsteile stumm schalten.
- **Bit-Maske** (Modus Bitwise): Mit einer festen Zahl auf `B` blendest du
  gezielt Bits eines Bytes aus. Beispiel `0F` (dezimal 15): Nur die unteren
  vier Bits von `A` bleiben übrig — alles andere wird 0. Probiere es aus:
...
Stelle `A` auf irgendeine Zahl und `B` auf 15 (Hex `0F`) — der Ausgang
behält nur die unteren vier Bits. Der Wert 15 wirkt hier wie eine Schablone.
